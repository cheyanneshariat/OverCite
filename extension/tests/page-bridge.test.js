...
test("readiness polling reuses only the exact immutable document and refreshes navigation metadata", async () => {
  await Promise.resolve();
  let serializations = 0;
  const doc = (text) => ({ length: text.length, toString() { serializations += 1; return text; } });
  const view = { state: { doc: doc("source"), selection: { main: { from: 0, to: 0 } } }, dispatch() {} };
  const editor = new FakeElement({ attributes: { class: "cm-editor" }, view });
  const tab = new FakeElement({ text: "main.tex Close", attributes: { role: "tab", "aria-selected": "true" } });
  const hooks = await loadBridgeHarness({ selectorMap: new Map([
    [".cm-editor", [editor]], ['[role="tab"][aria-selected="true"]', [tab]]
  ]) });
  const first = hooks.handleAction("getActiveEditorState", {});
  for (let i = 0; i < 100; i += 1) {
    const next = hooks.handleAction("getActiveEditorState", { knownDocumentRevision: first.documentRevision });
    assert.equal(next.text, undefined);
    assert.equal(next.documentRevision, first.documentRevision);
  }
  assert.equal(serializations, 1);
  tab.textContent = "refs.bib Close";
  const transitioning = hooks.handleAction("getActiveEditorState", { knownDocumentRevision: first.documentRevision });
  assert.equal(transitioning.fileName, "refs.bib");
  assert.equal(transitioning.documentRevision, first.documentRevision, "a new label must not invent new document contents");
  view.state.doc = doc("@article{Changed2026}");
  const changed = hooks.handleAction("getActiveEditorState", { knownDocumentRevision: first.documentRevision });
  assert.equal(changed.text, "@article{Changed2026}");
  assert.notEqual(changed.documentRevision, first.documentRevision);
  assert.equal(serializations, 2);
  const uncached = hooks.handleAction("getActiveEditorState", {});
  assert.equal(uncached.text, changed.text, "normal reads and write checks still receive full text");
});
